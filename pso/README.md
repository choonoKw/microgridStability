...
where $\boldsymbol{F}\_{\mathbf{x}}=\frac{\partial \boldsymbol{F}}{\partial \mathbf{x}}\Big|\_{(\mathbf x=\mathbf x^{\star},~\mathbf y=\mathbf y^{\star})}$ and  are the Jacobian matrices with respect to the state variables, and 
