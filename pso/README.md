...
Since the state matrix $A$  is a function of $\mathbf k$, the eigenvalues that determine the system's small-signal stability are also functions of $\mathbf k$, i.e., ${\lambda_i}(\mathbf{k})=\\{\text{eig}(A (\mathbf k))\\}\_i,\ i=1, 2, \cdots, n$. 
